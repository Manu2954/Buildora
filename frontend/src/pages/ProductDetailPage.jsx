...
import { getProductById } from '../services/storefrontService';
import { useCart } from '../context/CartContext';
import { Star, CheckCircle, Truck, Building, Minus, Plus, ShoppingCart, AlertTriangle, ChevronRight, Lock, RefreshCw, Zap } from 'lucide-react';
...
const StarRating = ({ rating = 4.5, count = 0 }) => (
    <div className="flex items-center">
        <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
                <Star key={i} size={20} className={i < Math.round(rating) ? 'text-yellow-400 fill-current' : 'text-gray-300 fill-current'} />
            ))}
        </div>
        {count > 0 && <span className="ml-3 text-sm text-gray-600">{count} ratings</span>}
    </div>
);
...
const ProductDetailPage = () => {
    ...
    const placeholderImage = `https://placehold.co/800x600/e2e8f0/475569?text=${encodeURIComponent(product.name)}`;
    ...
    const stock = selectedVariant ? selectedVariant.stock : product.stock.quantity;
            ...
            <div className="container px-4 py-8 mx-auto">
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <div className="lg:col-span-1"><div className="sticky top-24">
                        <div className="mb-4 overflow-hidden border border-gray-200 rounded-lg"><img src={images[selectedImage]} alt={product.name} className="object-contain w-full h-auto" onError={(e) => { e.target.src = placeholderImage; }}/></div>
                        <div className="flex -m-1">{images.map((img, index) => (<button key={index} onClick={() => setSelectedImage(index)} className={`w-20 h-20 p-1 m-1 overflow-hidden border rounded-md ${selectedImage === index ? 'border-indigo-500 ring-2 ring-indigo-200' : 'border-gray-200'}`}><img src={img} alt={`Thumbnail ${index + 1}`} className="object-contain w-full h-full" /></button>))}</div>
                    </div></div>
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <div className="mt-3"><StarRating count={254} /></div>
                        <hr className="my-6"/>
                            ...
                            <div className="flex items-baseline mt-1">
                                ...
                                {product.pricing?.mrp && (
                                    <div className="ml-3 text-gray-500">
                                        ...
                                        <span className="text-sm line-through">₹{product.pricing.mrp.toFixed(2)}</span>
                                    </div>
                                )}
                            </div>
                                ...
                                <div className="flex flex-wrap gap-3 mt-4">
                                    {product.variants.map((variant) => (
                                        <button
                                            ...
                                            className={`relative px-4 py-2 text-sm font-medium border rounded-lg transition-all duration-200 ${
                                                selectedVariant?._id === variant._id
                                                    ? 'bg-indigo-600 text-white border-indigo-600 ring-2 ring-offset-2 ring-indigo-500'
                                                    : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                                            } ${variant.stock === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed border-dashed' : ''}`}
                                        >
                             ...
                             <table className="w-full mt-2 text-sm text-left text-gray-700">
                                <tbody>
                                    ...
                                    {product.attributes && product.attributes.map(attr => (
                                         <tr key={attr.name} className="border-b"><td className="py-2 font-medium text-gray-500">{attr.name}</td><td className="py-2">{attr.value}</td></tr>
                                    ))}
                                    {product.dimensions && (product.dimensions.length || product.dimensions.width || product.dimensions.height) && (
                                        <tr className="border-b"><td className="py-2 font-medium text-gray-500">Dimensions</td><td className="py-2">{`${product.dimensions.length || 'N/A'} x ${product.dimensions.width || 'N/A'} x ${product.dimensions.height || 'N/A'} ${product.dimensions.unit}`}</td></tr>
                                    )}
                                    {product.weight?.value && (
                                         <tr className="border-b"><td className="py-2 font-medium text-gray-500">Weight</td><td className="py-2">{`${product.weight.value} ${product.weight.unit}`}</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
