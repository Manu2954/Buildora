...
const ProductDetailPage = () => {
  ...
  const images = product.images?.length
    ? product.images
    : [
        `https://placehold.co/800x600/e2e8f0/475569?text=${encodeURIComponent(
          product.name
        )}`,
      ];
      ...
      <div className="container px-4 py-8 mx-auto">
              ...
              <div className="flex -m-1">
                {images.map((img, index) => (
                  <button
                    ...
                    className={`w-20 h-20 p-1 m-1 overflow-hidden border rounded-md ${
                      selectedImage === index
                  ...
                  {product.dimensions &&
                    (product.dimensions.length ||
                      ...
                      <tr className="border-b">
                        ...
                        } x ${product.dimensions.width || "N/A"} x ${
                          product.dimensions.height || "N/A"
                        } ${product.dimensions.unit}`}</td>
                      </tr>
                    )}
                  {product.weight?.value && (
                    <tr className="border-b">
                      <td className="py-2 font-medium text-gray-500">Weight</td>
                      <td className="py-2">{`${product.weight.value} ${product.weight.unit}`}</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="lg:col-span-1">
            <div className="p-4 border border-gray-200 rounded-lg shadow-sm sticky top-24">
              ...
              <button
                ...
                className="flex items-center justify-center w-full px-8 py-3 mt-6 text-base font-medium text-black bg-yellow-400 border border-transparent rounded-full hover:bg-yellow-500 disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
                {isAdding ? "Adding..." : "Add to Cart"}
              </button>
              <button
                disabled={stock === 0}
                className="flex items-center justify-center w-full px-8 py-3 mt-3 text-base font-medium text-black bg-orange-400 border border-transparent rounded-full hover:bg-orange-500 disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
                Buy Now
              </button>
        ...
        <div className="pt-16 mt-16 border-t">
          ...
          <section className="mt-16">
            ...
            {isAuthenticated && hasUserReviewed && (
              <p className="p-4 mt-4 text-sm text-blue-700 bg-blue-50 rounded-md">
                You've already reviewed this product.
              </p>
            )}
            {!isAuthenticated && (
              <p className="p-4 mt-4 text-sm text-gray-700 bg-gray-100 rounded-md">
                <Link
                  to="/login"
