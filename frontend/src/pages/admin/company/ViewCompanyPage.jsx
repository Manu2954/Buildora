...
import { Building, Mail, Phone, Globe, MapPin, Edit3, Trash2, PlusCircle, Package, AlertTriangle, ChevronLeft } from 'lucide-react';
...
const ViewCompanyPage = () => {
    ...
    const [productsData, setProductsData] = useState({ companyName: '', products: [] }); // API returns { companyName, products }
    ...
    const fetchCompanyAndProducts = useCallback(async () => {
        ...
        try {
            ...
            if (err.status === 404) {
                 setError(`Company with ID ${companyId} not found. You may need to go back and select a valid company.`);
            }
        } finally {
            setIsLoadingCompany(false);
        }
        ...
        try {
            ...
            setProductsData(productsResponse || { companyName: company?.name || 'N/A', products: [] }); // Ensure products array exists
        } catch (err) {
            console.error("Failed to fetch products for company:", err);
            // Don't overwrite company error if products fail
            if (!error) setError(err.message || "Could not load products for this company.");
        } finally {
            setIsLoadingProducts(false);
        }
    ...
    const confirmDeleteProduct = async () => {
        ...
        try {
            ...
            const updatedProductsResponse = await getProductsForCompany(companyId, token);
            setProductsData(updatedProductsResponse || { companyName: company?.name || 'N/A', products: [] });
        } catch (err) {
        ...
        return (
            <div className="flex items-start py-2">
                ...
                <div>
                    ...
                    {isLink ? (
                        <a href={value.startsWith('http') ? value : `https://${value}`} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-600 hover:underline">
                            {value}
                        </a>
                    ) : (
    ...
    if (error && !company) { // Critical error if company itself cannot be loaded
        return (
             <div className="container px-4 py-8 mx-auto">
                ...
                <div className="p-8 text-center text-red-700 bg-red-100 border-2 border-red-300 rounded-lg shadow-md">
                    ...
                    <h2 className="mb-2 text-2xl font-semibold">Error Loading Company Details</h2>
    ...
    const generalErrorDisplay = error && company ? (
        <div className="p-4 my-4 text-sm text-orange-700 bg-orange-100 border border-orange-400 rounded-md" role="alert">
            <strong className="font-bold">Notice:</strong>
            <span className="block sm:inline"> {error} (Company details might be visible, but product data may be affected)</span>
             <button onClick={fetchCompanyAndProducts} className="ml-2 text-sm font-semibold text-orange-800 underline">Retry fetching all data.</button>
        </div>
    ) : null;


    const addressString = company?.address 
        ? `${company.address.street || ''}, ${company.address.city || ''}, ${company.address.state || ''} ${company.address.postalCode || ''}, ${company.address.country || ''}`.replace(/, ,/g,',').replace(/^,|,$/g,'').trim()
        : 'Not provided';
                ...
                <div className="p-6 mb-8 bg-white rounded-lg shadow-xl">
                    <div className="flex flex-col items-start justify-between gap-4 mb-6 md:flex-row md:items-center">
                        <div className="flex items-center">
                            {company.logoUrl ? (
                                <img src={company.logoUrl} alt={`${company.name} logo`} className="w-16 h-16 mr-4 rounded-md object-contain border" 
                                onError={(e) => { e.target.onerror = null; e.target.src="https://placehold.co/64x64/e2e8f0/94a3b8?text=Logo"; }}/>
                            ) : (
            ...
            <div className="p-6 bg-white rounded-lg shadow-xl">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">
                        Products by {productsData.companyName || company?.name || 'this Company'}
                    </h2>
                    <Link
                ...
                {isLoadingProducts && (
                    <div className="flex items-center justify-center py-10">
                        <svg className="w-8 h-8 text-indigo-600 animate-spin" /* ... */ ></svg>
                        <p className="ml-2 text-gray-600">Loading products...</p>
                    </div>
                )}

                {!isLoadingProducts && productsData.products && productsData.products.length === 0 && (
                     <div className="py-10 text-center text-gray-500">
                        <Package size={48} className="mx-auto mb-2" />
                        <p className="mb-2 text-lg">No products found for this company.</p>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {productsData.products.map((product) => (
                                    <tr key={product._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{product.sku || 'N/A'}</td>
                                        <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{product.category}</td>
                                        <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                                            {product.pricing?.basePrice ? `₹${product.pricing.basePrice.toFixed(2)}` : 'N/A'}
                                        </td>
                                        <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{product.stock?.quantity ?? 'N/A'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <Link to={`/admin/companies/${companyId}/products/edit/${product._id}`} className="p-1 text-blue-600 hover:text-blue-900" title="Edit Product">
                                                <Edit3 size={18} />
                                            </Link>
                                            <button onClick={() => handleDeleteProductClick(product)} className="p-1 ml-2 text-red-600 hover:text-red-900" title="Delete Product">
                                                <Trash2 size={18} />
                                            </button>
                                        </td>
                                    </tr>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
                    <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-xl">
                        ...
                        <p className="mb-6 text-sm text-gray-600">
                            Are you sure you want to delete the product "<strong>{productToDelete.name}</strong>" from <strong>{company?.name}</strong>? This action cannot be undone.
                        </p>
